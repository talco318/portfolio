        ...
        <div className="mt-12 w-full">
            {!isOpen ? (
                <motion.button
                    ...
                    className="flex items-center gap-2 px-6 py-3 bg-gray-900 border border-gray-700 rounded-lg text-green-400 font-mono hover:bg-gray-800 transition-colors mx-auto md:mx-0 shadow-lg"
                ...
                <motion.div
                    ...
                    className="w-full max-w-3xl bg-[#1e1e1e] rounded-xl overflow-hidden shadow-2xl border border-gray-700 font-mono text-sm md:text-base"
                    onClick={() => inputRef.current?.focus()}
                >
                    {/* Mac-style Window Top Bar */}
                    <div className="flex items-center px-4 py-3 bg-[#2d2d2d] border-b border-gray-800">
                        <div className="flex gap-2">
                            <button onClick={() => setIsOpen(false)} className="w-3 h-3 rounded-full bg-red-500 hover:bg-red-600 transition-colors" title="Close" />
                            <div className="w-3 h-3 rounded-full bg-yellow-500" />
                            <div className="w-3 h-3 rounded-full bg-green-500" />
                        </div>
                        <div className="mx-auto text-gray-400 text-xs tracking-wider">talcohen — bash — 80x24</div>
                    </div>

                    {/* Terminal Window */}
                    <div ref={containerRef} className="p-4 h-64 md:h-96 overflow-y-auto text-gray-300 scroll-smooth">
                        <AnimatePresence>
                            {lines.map((line, i) => (
                                <motion.div
                                    key={i}
                        ...
                        <div className="flex items-center mt-2 group">
                            <span className="text-blue-400 mr-2 shrink-0">guest@talcohen:~$</span>
                            <input
                                ref={inputRef}
                                type="text"
                                value={input}
                                ...
                                className="bg-transparent outline-none flex-1 text-gray-300 caret-white"
                                spellCheck="false"
                                autoComplete="off"
                            />
                        </div>
